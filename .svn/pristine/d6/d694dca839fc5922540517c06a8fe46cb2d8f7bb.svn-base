...
(function(global, jQuery, undefined) {
	...
	if (mt_mtBpParams) {
		...
		mt_mtBpParams.noBuryElemets = [ 'a',
				'input[type!=button][type!=radio][type!=checkbox]|blur',
	...
	function mtBPNobury(e) {
		...
		var content = {
			...
			'NoBuryTitle' : jQuery(this).text() ? jQuery(this).text() : jQuery(
					this).attr('title') ? jQuery(this).attr('title') : jQuery(
					this).val(),
			'NoBuryExtend' : ''
		};
		// 根据不同的元素获取值
	...
	function pageNoBuryFunc() {
		if (mt_mtBpParams.noBuryKey && mt_mtBpParams.noBuryKey != '') {
			for (var i = 0; i < mt_mtBpParams.noBuryElemets.length; i++) {
				...
				var eventName = vals[1] ? vals[1] : mt_mtBpParams.noBuryEvent;//
				//指定事件则取否则使用默认事件
				if (selectKit) {
	...
	// 3:以上方法前提条件：
	// 3.1:确定ga方法在页面调用的格式都为：ga('send', 'event', 'ershoufangliebiao',
	// 'fangyuanbiaoti', '1_1');
